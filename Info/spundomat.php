...
list($isCalib, $dens, $temperature, $angle, $battery) = getChartValuesMQTTDevice($conn, $_GET['name'], $timeFrame, $_GET['moving'],  $_GET['reset']);
...
if (!$_GET['reset'])
{
 $DataAvailable=isDataAvailable($conn, $_GET['name'], $timeFrame);
  ...
  {
   $Header='Spundomat: ' . $header_no_data_1 . ' ' . $_GET['name']. ' ' . $header_no_data_2 . ' ' .$DataAvailable[1]. ' ' . $header_no_data_3;
  }
}
...
<html>
<head>
  <title>MQTTDevice Data</title>
  <meta http-equiv="refresh" content="120">
  <meta name="Keywords" content="Spundomat, Chart, genericTCP">
  <meta name="Description" content="Spundomat Chart">
...
const chartBatt=[<?php echo $battery;?>]
        ...
        chart = new Highcharts.Chart(
        {
            ...
            [
                ...
                },
				{
                    name: 'Pressure',
                    color: '#00AB66',
                    data: chartBatt.map(row => [row.timestamp, row.value])
                }
            ] //series      
            });
    }
